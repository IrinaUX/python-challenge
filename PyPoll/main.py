...
def create_candidates_list(path, output_file):
    ...
    with open(path, 'r', newline='') as file_input: # open('output.txt', 'wb') as file_output:
        ...
        #print(Counter(row[index['Candidate']] for row in csv_reader_object if row[index['Candidate']] == '').most_common())

        # for row in csv_reader_object:
        #     key = row
        #     if key not in candidates_list:
        ...
        #print(f"Number of occurences for the candidate: {candidates['Khan']}.")
...
print(f"The list of all the candidates: {candidates_list}.")